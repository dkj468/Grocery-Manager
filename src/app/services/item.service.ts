...
export class ItemService{
  ...
  addItem(itemName: string, itemQuantity: number, itemUnit: string){
    let id = 1;
    if(this.items.length > 0){
      id = this.items[this.items.length -1].id + 1;
    }
    const item: Item = {id:id, itemName: itemName, itemQuantity: itemQuantity, itemUnit: itemUnit, itemAmount: 0}
  ...
  getTotalAmount(){
    let totalAmount = 0;
    this.items.forEach(el => totalAmount += el.itemAmount);
    return totalAmount;
  }
